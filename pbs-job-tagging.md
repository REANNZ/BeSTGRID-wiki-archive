...
These changes are summarized by the following patch to `/etc/cron.hourly/auditquery` ([auditquery-fix-NULL-id.diff](attachments/Auditquery-fix-NULL-id.diff.txt), file info: 
...
- Download the script [GetJobDN.sh](attachments/GetJobDN.sh.txt) and store it as `/opt/vdt/globus/lib/perl/Globus/GRAM/JobManager/GetJobDN.sh`
...
- Download the patch [pbs.pm-tag-dn-from-audit.diff](attachments/Pbs.pm-tag-dn-from-audit.diff.txt) and use it to patch `/opt/vdt/globus/lib/perl/Globus/GRAM/JobManager/pbs.pm`
...
echo     "`date` Job-DN: $Line" >> /opt/vdt/globus/var/pbs-acct/jobdn.log
```
- Install the fix to `auditquery` described above in [Avoid loss of JobDN data](#PBSjobtagging-Fixingauditquery___avoidlossofJobDNdata) - apply the patch [auditquery-fix-NULL-id.diff](attachments/Auditquery-fix-NULL-id.diff.txt) to `/etc/cron.hourly/auditquery`
...
```
- Note: the `jobdn-subm.log` must be world-writable, because pbs.pm runs with the permission of the local user account to which the grid user is mapped.  On the contrary, jobdn.log is written to only by auditquery running as root, so no access permissions have to be granted.
- Download the perl module [getcertndn.pm](attachments/Getcertdn.pm.txt) and store it as `/opt/vdt/globus/lib/perl/Globus/GRAM/JobManager/getcertdn.pm`
- Download the patch [pbs.pm-tag-dn-email-log-jobdn.diff](attachments/Pbs.pm-tag-dn-email-log-jobdn.diff.txt) and use it to patch `/opt/vdt/globus/lib/perl/Globus/GRAM/JobManager/pbs.pm`
- Modify auditquery as indicated above: insert the following line at the beginning of the while loop, just above the logger command:
...
The patch based on GRAM audit database is [pbs.pm-tag-dn-from-audit.diff](attachments/Pbs.pm-tag-dn-from-audit.diff.txt).

The old patch, based on certificates, is [pbs.pm-tag-dn-email-log-jobdn.diff](attachments/Pbs.pm-tag-dn-email-log-jobdn.diff.txt) 
